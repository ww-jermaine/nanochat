...
LOG_FILE="${TIMESTAMP}_gpu_usage.csv"
echo "stage,timestamp,utilization.gpu,utilization.memory,memory.used,memory.total,power.draw,temperature.gpu" > "$LOG_FILE"
...
start_gpu_monitor () {
  ...
  nvidia-smi \
    --query-gpu=timestamp,index,utilization.gpu,power.draw,clocks.sm,memory.used \
    --format=csv,noheader -l 1 \
  | awk -v stage="$STAGE_NAME" 'BEGIN { OFS="," } { print stage, $0 }' \
  >> "$OUT_FILE" &
...
start_gpu_monitor "$STAGE" "$LOG_FILE"
